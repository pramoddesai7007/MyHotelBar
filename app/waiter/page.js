...
const Waiter = () => {
  ...
  const handleAddWaiter = async () => {
    ...
    if (waiterData.contactNumber.length !== 10 || isNaN(waiterData.contactNumber)) {
      ...
      const response = await axios.post('http://103.159.85.246:6001/api/waiter', waiterData);
  ...
  //   try {
  //     const response = await axios.put(`http://103.159.85.246:6001/api/waiter/${waiterId}`, waiterData);
  ...
  const confirmDeleteWaiter = async () => {
    try {
      await axios.delete(`http://103.159.85.246:6001/api/waiter/${deletingWaiterId}`);
