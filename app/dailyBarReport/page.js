...
const DailyBarReport = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await axios.get(
        `http://103.159.85.246:6001/api/order/ordersListBar/date?startDate=${startDate}&endDate=${endDate}`
      );
...
<tr class="bg-gray-100 table_downward">
  ...
  const handleUpdateOrders = async () => {
    ...
    try {
      ...
      const response = await axios.put("http://103.159.85.246:6001/api/order/orders/flag");
