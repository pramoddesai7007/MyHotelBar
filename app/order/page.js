...
import { faCirclePlus, faCircleMinus, faTimes, faTableColumns, faObjectUngroup, faHouse, faObjectGroup, faLeftRight } from "@fortawesome/free-solid-svg-icons";
...
const Billing = ({ tableId, acPercentage }) => {
  ...
  const handleCancelKOT = async () => {
    try {
      ...
      await axios.delete(`http://103.159.85.246:6001/api/kot/${tableId}`, {
  ...
  const saveBill = async () => {
    try {
      const stockResponse = await axios.get('http://103.159.85.246:6001/api/liquorBrand/liquorBrand/stock');
  ...
  <div class="container">
    ...
    <img class="logo" src="http://103.159.85.246:6001/${hotelInfo.hotelLogo
...
<script>
  ...
  const handleShiftTables = async (sectionId, destinationTableName, sourceTableName) => {
    try {
      // Perform a lookup in the Table collection to retrieve tableId for destinationTableName
      const destinationTableResponse = await axios.get(`http://103.159.85.246:6001/api/table/table/bySectionAndName/${sectionId}/${destinationTableName}`);
      const destinationTableId = destinationTableResponse.data._id;

      // Perform a lookup in the Table collection to retrieve tableId for sourceTableName
      const sourceTableResponse = await axios.get(`http://103.159.85.246:6001/api/table/table/bySectionAndName/${sectionId}/${sourceTableName}`);
      const sourceTableId = sourceTableResponse.data._id;

      // Call the mergeTables endpoint with the retrieved tableIds
      const response = await axios.patch('http://103.159.85.246:6001/api/order/shiftBills', {
        destinationTableId,
        sourceTableId,
      });
      console.log(response.data); // Handle response data as needed
      ...
      {showPopup && (
        <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-4 shadow-2xl z-50 rounded-lg border border-blue-700">
          <div className="text-center">
            <p className="mb-4">
              yStock Quantity is not available for{" "}
              <b>
                <i>{ProductName}</i>
              </b>
              !{" "}
        ...
        <div className="flex lg:flex-row shadow-lg">
          ...
          <div className=" w-56 lg:w-3/5 md:w-96 lg:pl-3 hidden md:block bg-white ">
            ...
            <div className=" flex flex-row px-2 ml-1 custom-scrollbars overflow-x-auto whitespace-nowrap mt-1">
              <span
                key="all-items"
                className={`cursor-pointer px-2 bg-orange-100 py-1 mb-1 rounded-xl text-xs lg:text-sm font-semibold mr-4 ${selectedBarCategory === null ? "bg-yellow-500 text-white" : ""
                  }`}
                onClick={() => handleBarCategoryClick(null)}
              >
                All Bar Menu
              ...
              {barCategories.map((category) => (
                <span
                  key={category._id}
                  className={`whitespace-nowrap bg-orange-100 cursor-pointer px-5 ml-3 py-1 mb-1 rounded-xl lg:text-sm md:text-sm text-xs sm:text-xs font-semibold ${selectedBarCategory === category
                    ? "bg-yellow-500 text-white"
                    : ""
                    }`}
                  onClick={() => handleBarCategoryClick(category)}
