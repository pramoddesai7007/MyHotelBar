...
const ExpenseFormList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const expenseResponse = await axios.get('http://103.159.85.246:6001/api/expense/expenses');
        setExpenses(expenseResponse.data);

        const bankResponse = await axios.get('http://103.159.85.246:6001/api/bankName/bankNames');
  ...
  const fetchDataForTable = async () => {
    try {
      const tableResponse = await axios.get('http://103.159.85.246:6001/api/expensesForm');
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      const response = await axios.patch(
        `http://103.159.85.246:6001/api/expensesForm/${editExpense._id}`,
        {
          date: expenseDate,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://103.159.85.246:6001/api/expensesForm', {
        date: expenseDate,
        expenseType: selectedExpense,
        description: description,
        paidBy: paidBy,
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(`http://103.159.85.246:6001/api/expensesForm/${deleteExpense._id}`);
