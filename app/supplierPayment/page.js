...
const SupplierPayment = () => {
  ...
  useEffect(() => {
    axios.get('http://103.159.85.246:6001/api/supplier/suppliers')
  ...
  useEffect(() => {
    if (selectedSupplier) {
      axios.get(`http://103.159.85.246:6001/api/supplier/suppliers/${selectedSupplier}`)
        .then(response => {
  ...
  const handleDebitSubmission = (e) => {
    ...
    if (debitAmount <= creditBalance && !loading) {
      ...
      axios.put(`http://103.159.85.246:6001/api/supplier/updateBalance/${selectedSupplier}`, {
        debit: debitAmount,
      })
        .then(response => {
  ...
  const handleDeleteConfirmed = async () => {
    try {
      ...
      await axios.delete(`http://103.159.85.246:6001/api/supplier/suppliers/${selectedSupplier}/dateWiseRecords/${recordIdToDelete}`);
